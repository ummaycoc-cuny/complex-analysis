...
\remark The \term{free abelian group generated by $S$} is the collection of functions $\varphi : S \to \mathbb{Z}$ with $\varphi$ non-zero finitely often. $\varphi = n_1 \varphi_{x_1} + \cdots + n_m \varphi_{x_m}$ where $\varphi$ vanishes outside $\{x_1, \ldots, x_m\}$ and $n_k = \varphi(x_k)$, and $\varphi_x$ sends $x$ to $1$ and vanishes elsewhere. Such representations are unique, and identifying $\varphi_x$ with $x$ we get $\varphi = n_1 x_1 + \cdots + n_m x_m$.

\remark Let $U \subseteq \mathbb{C}$ be non-empty and open. 
